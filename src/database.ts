...
export async function getAllRelations(conn): Promise<string[]> {
  ...
  conn.from('relations').select("*")
      .then((rows) => 
        rows.forEach(row => {
          relationsArray.push(`${row['id']} ${row['first_class']} ${row['relation']} ${row['second_class']}`);
...
export async function getAllInheritanceRelations(conn): Promise<string[]> {
  ...
  conn.from('relations').select("*").where({ relation: "extension"})
  .then((rows)=>
  rows.forEach(row => {
    relationsArray.push(`${row['id']} ${row['first_class']} ${row['relation']} ${row['second_class']}`);
  })).catch((e)=> {console.log(e); throw e});
...
export async function getAllWithRelation(relationName: string, conn): Promise<string[]> {
  ...
  conn.from('relations').select("*").where({ relation: relationName})
  .then((rows)=>
  rows.forEach(row => {
    relationsArray.push(`${row['id']} ${row['first_class']} ${row['relation']} ${row['second_class']}`);
  })).catch((e)=> {console.log(e); throw e});
