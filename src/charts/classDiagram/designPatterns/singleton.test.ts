...
import { checkSingletonByName, createDesignPatternTable, getAllDesignPatterns } from "./singleton";
...
describe("Singleton tests", () => {
  ...
  test("Check singleton", async () => {
    expect(await checkSingletonByName("Singleton", conn)).toStrictEqual(true);
    expect(await checkSingletonByName("Animal", conn)).toStrictEqual(false);
    expect(await checkSingletonByName("inexistentClass", conn)).toStrictEqual(
      false
    );
  });
  ...
  test("Get all design patterns", async() => {
    await getAllDesignPatterns(conn).then(res => {
        expect(JSON.stringify(res)).toStrictEqual(JSON.stringify(dPatterns));
    })
  })
});
